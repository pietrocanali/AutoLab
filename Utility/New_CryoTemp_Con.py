...
THE LAKESHORE 335 IS REFERRED TO AS Tcon (Temperature controller)
...
from Utility import Monitor335_Window
...
class Util(tk.Frame):
    ...
    def __init__(self, master, parent):
        ...
        master.add(Utilframe,text="Temp Reading lakeshore 335")
        ...
        gbipLabel_335 = tk.Label(Utilframe,text="335 GPIB")
        gbipLabel_335.grid(column=0, row=0, pady=5)
        self.gpib335Entry = tk.Entry(Utilframe,width = 5)
        self.gpib335Entry.insert(tk.END,"5")
        self.gpib335Entry.grid(column=1, row=0, pady=5)
    ...
    def Connect(self):
        ...
        L335add=self.gpib335Entry.get()
        #L218add=int(self.gpib218Entry.get())#lakeshore addresses
        try:
            self.Tcon=Inst.lakeshore335(self.rm,int(L335add))
    ...
    def set_Setpoint(self, pipe=None):
        """
        If connected, set the Setpoint on the VTI on the 335. Using Loop/Output 1
    ...
    def Alloff(self):
        if self.Isconnected == False:
            ...
            L335add=self.gpib335Entry.get()
            Tcon=Inst.lakeshore335(self.rm,int(L335add))
            Tcon.allOff()
            Tcon.close()
            rm.close()
        else:            
    ...
    def Start_monitoring(self):
        ...
        #addresses=[int(self.gpib335Entry.get()),int(self.gpib218Entry.get())]
        addresses=[int(self.gpib335Entry.get())]
        self.parent.Temperature_Monitor=Monitor335_Window.Mon_Win(self,self.parent,addresses)
