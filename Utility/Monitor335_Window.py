...
class Mon_Win(tk.Frame):
    ...
    SensorInput=["A","B","C","D"]
    TC_Model=["Lakeshore 335"]#Temperature controller Models ## why is this both?
    TM_Model=["Lakeshore 218", "Lakeshore 350"]#temperature Monitor models
    Heater_Range=["Off","Low","Med","High"]
    #Delete as appropraite when you know the power
    Temperature_Data=[]#empty list to append data to
    Time_Data=[]
    Colour_List=["#a10000",'#416600',"#626262","#a15000","#a1a100","#008141","#008282","#005682","#000056","#2b0057","#6a006a","#77003c"]
    ...
    def __init__(self, master,parent,default_addresses):
        ...
        addresses=self.parent.address_list #where is this from? and what are the default adresses?
        ...
        self.Model=tk.StringVar(Control_Frame,"Lakeshore 335")
...
# =============================================================================
        ...
        self.StartButton.grid(column=7,row=1,rowspan=6)
        
        self.SaveData= tk.Button(Control_Frame,
                                 text= 'Save Data',
                                 command= lambda: self.UpdateFile())
        
        self.SaveData.grid(column=7, row=0)
...
# =============================================================================
        ...
        #print(default_addresses)
        self.Open_Pipes(default_addresses[0])
        self.closed=False
# =============================================================================
...
# =============================================================================
    ...
    def close_function(self):
        ...
        #self.fig.clear() #just to be sure graph does not keep updating
        self.Temp_PipeRecv.send("STOP")
        #self.Temp_PipeSend.send('Esc')
        self.closed=True
        self.Window.destroy()
# =============================================================================
    ...
    def UpdateGraph(self):
        ...
        try:
            ...
            if len(self.Time_Data) <=10000:
                ...
                self.Time_Data.append(self.current_Data[0])
                self.Temperature_Data.append(self.current_Data[1::])
            for x in range (0,2):#Temp A, Temp B on 335
...
# SAVE DATA FUNCTION
    ...
    def UpdateFile(self):
        ...
        hour=daytime[3].split(':')
        
        filename= f'Data/data_{daytime[1]}_{daytime[2]}_{hour[0]}00.txt'
        
        self.Get_Pipe_Data()
        self.DataFile(filename)
        self.after(2500,self.UpdateFile)
        

    def DataFile(self, filename):
        '''Save temp, heater range, setpoint, time, pid... in a file'''
  #      if save == False:
   #         #self.SaveData.configure(command=self.DataFile(True))
    #        return None
     #   else:
      #      pass
        #time.sleep(10)
        ...
        self.data = 'Time: {}; TempA: {}; TempB: {}'.format(self.current_Data[0],
                                                            self.current_Data[1],
                                                            self.current_Data[2])
        with open(filename, 'a') as datafile:
            #datafile.write('#Time TempA TempB\n')
            #for line in datafile.readlines():
            #if line=='':
            datafile.write(self.data+'\n')
            #else:
             #       None
            #datafile.write(str(time.time()))
        #print('saved ' +str(time.ctime()))
            #startT = time.time()
...
def Controller(Pipe,TCon_add,Backup_TConAdd, TMon_add=None):
    ...
    Currently Assumes that you're doing Loop 1 connected to sensor B (head)
    ...
    #print(rm.resource_info(rm.list_resources()[-1]))
    Abort = False
    ...
    try:
        #print('will try rm')
        #print(TCon_add)
        T_Con = Inst.lakeshore335(rm,TCon_add)
        if TMon_add != None:
            T_Mon=Inst.lakeshore218(rm,TMon_add) #are there two temp controllers? thi is a monitor
        else:
            T_Mon=None
    ...
    while Abort == False:
        ...
        Current_TCon_head=T_Con.getTempN("B")
        Current_TCon_Sample=T_Con.getTempN("A")
        if T_Mon != None:
            Current_TMon=T_Mon.getTempAll()
            
        else:
            Current_TMon=[]
        Pipe.send([time.time(),Current_TCon_head,Current_TCon_Sample,*Current_TMon,IsRamping,IsStable])#sends the current reading of the pipes to be read
...
# =============================================================================
    ...
    # =============================================================================
                ...
                #Yes this does allow you to Send "T; Red Dragon Archfiend" as a valid get command but SHHH. 
                    if re.search("HMD",Param[0]): #what is HMD? and why is a get command len2?
                        mode=T_Con.getOutputMode(1)[0]
                        if mode==3:
                            mode="MO" #I think this is open loop
                        elif mode==2:
                            mode="Z" #zone
                        elif mode==1:
                            mode="MP" #closed loop PID
                        Pipe.send(mode)
    ...
    # =============================================================================
                    ...
                    #print(Param[2])
                    if re.search("HMD",Param[0]): #and Param1 is an equal sign
                        
                        if re.search("MO",Param[2]):
                            T_Con.setOutputMode(3,0) #open loop
                        elif re.search("Z",Param[2]):
                            T_Con.setOutputMode(2,0) #zone
                        elif re.search("MP",Param[2]):
                            T_Con.setOutputMode(1,0) #closed loop pid
                        ...
                        T_Con.setRampRate(1,float(Param[2]))
                    else:
